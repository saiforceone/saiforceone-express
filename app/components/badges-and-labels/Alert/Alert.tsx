...
import {BsCheckSquareFill, BsExclamationSquareFill, BsX} from "react-icons/all";
...
const getIcon = (status: PackageStatus): React.ReactElement => {
  switch (status) {
    case PackageStatus.customsDetained:
      return <BsExclamationSquareFill className={commonIconClasses} size={commonIconSize} />;
    case PackageStatus.delivered:
      return <BsCheckSquareFill className={commonIconClasses} size={commonIconSize} />;
    default:
      return <BsInfoSquareFill className={commonIconClasses} size={commonIconSize} />;
  }
}
...
export const Alert: FC<AlertProps> = ({alertTitle, alertText, dismissAction, packageStatus}) => {
  return (
